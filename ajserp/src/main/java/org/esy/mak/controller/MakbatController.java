 ...
 */
@Controller
@RequestMapping("/api/mak/makbat")
public class MakbatController {

    public static final String AUTHORITY = "mak_makbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IMakbatService makbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Makbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", makbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Makbat o = makbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Makbat, pageable
	 * @return HttpResult
	 * @date Fri Jul 31 14:34:04 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Makbat makbat, Pageable pageable) {
		if (makbat == null) {
			makbat = new Makbat();
		}
		return new HttpResult(makbatService.query(makbat, pageable));
	}
