 ...
 */
@Controller
@RequestMapping("/api/mak/makbah")
public class MakbahController {

    public static final String AUTHORITY = "mak_mapm1";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IMakbahService makbahService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Makbahv o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", makbahService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Makbahv o = makbahService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Makbah, pageable
	 * @return HttpResult
	 * @date Fri Jul 31 14:33:59 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Makbahv makbahv, Pageable pageable) {
		if (makbahv == null) {
			makbahv = new Makbahv();
		}
		return new HttpResult(makbahService.query(makbahv, pageable));
	}
