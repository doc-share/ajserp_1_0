 ...
 */
@Controller
@RequestMapping("/api/pur/venchkbah")
public class VenchkbahController {

    public static final String AUTHORITY = "rec_map5a";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IVenchkbahService venchkbahService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Venchkbahv o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", venchkbahService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Venchkbahv o = venchkbahService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Venchkbah, pageable
	 * @return HttpResult
	 * @date Wed Aug 05 14:12:46 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Venchkbahv venchkbahv, Pageable pageable) {
		if (venchkbahv == null) {
			venchkbahv = new Venchkbahv();
		}
		return new HttpResult(venchkbahService.query(venchkbahv, pageable));
	}
