 ...
 */
@Controller
@RequestMapping("/api/pur/recbat")
public class RecbatController {

    public static final String AUTHORITY = "pur_recbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IRecbatService recbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Recbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", recbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Recbat o = recbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Recbat, pageable
	 * @return HttpResult
	 * @date Wed Jul 29 11:40:38 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Recbat recbat, Pageable pageable) {
		if (recbat == null) {
			recbat = new Recbat();
		}
		return new HttpResult(recbatService.query(recbat, pageable));
	}
