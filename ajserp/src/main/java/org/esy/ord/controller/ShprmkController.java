 ...
 */
@Controller
@RequestMapping("/api/ord/shprmk")
public class ShprmkController {

    public static final String AUTHORITY = "ord_shprmk";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IShprmkService shprmkService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Shprmk o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", shprmkService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Shprmk o = shprmkService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Shprmk, pageable
	 * @return HttpResult
	 * @date Tue Jul 28 15:23:53 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Shprmk shprmk, Pageable pageable) {
		if (shprmk == null) {
			shprmk = new Shprmk();
		}
		return new HttpResult(shprmkService.query(shprmk, pageable));
	}
