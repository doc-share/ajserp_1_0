 ...
 */
@Controller
@RequestMapping("/api/ord/quobat")
public class QuobatController {

    public static final String AUTHORITY = "ord_quobat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IQuobatService quobatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Quobat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", quobatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Quobat o = quobatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Quobat, pageable
	 * @return HttpResult
	 * @date Mon Jul 27 15:34:25 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Quobat quobat, Pageable pageable) {
		if (quobat == null) {
			quobat = new Quobat();
		}
		return new HttpResult(quobatService.query(quobat, pageable));
	}
