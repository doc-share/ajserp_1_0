 ...
 */
@Controller
@RequestMapping("/api/ord/cuschkbah")
public class CuschkbahController {

    public static final String AUTHORITY = "shp_map4a";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private ICuschkbahService cuschkbahService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Cuschkbahv o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", cuschkbahService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Cuschkbahv o = cuschkbahService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Cuschkbah, pageable
	 * @return HttpResult
	 * @date Wed Aug 05 11:46:33 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Cuschkbahv cuschkbahv, Pageable pageable) {
		if (cuschkbahv == null) {
			cuschkbahv = new Cuschkbahv();
		}
		return new HttpResult(cuschkbahService.query(cuschkbahv, pageable));
	}
