 ...
 */
@Controller
@RequestMapping("/api/ord/quobah")
public class QuobahController {

    public static final String AUTHORITY = "quo_mapa1";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IQuobahService quobahService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Quobahv o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", quobahService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Quobahv o = quobahService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Quobah, pageable
	 * @return HttpResult
	 * @date Mon Jul 27 15:15:17 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Quobahv quobahv, Pageable pageable) {
		if (quobahv == null) {
			quobahv = new Quobahv();
		}
		return new HttpResult(quobahService.query(quobahv, pageable));
	}
