 ...
 */
@Controller
@RequestMapping("/api/ord/cuschkbat")
public class CuschkbatController {

    public static final String AUTHORITY = "ord_cuschkbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private ICuschkbatService cuschkbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Cuschkbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", cuschkbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Cuschkbat o = cuschkbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Cuschkbat, pageable
	 * @return HttpResult
	 * @date Wed Aug 05 11:46:37 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Cuschkbat cuschkbat, Pageable pageable) {
		if (cuschkbat == null) {
			cuschkbat = new Cuschkbat();
		}
		return new HttpResult(cuschkbatService.query(cuschkbat, pageable));
	}
