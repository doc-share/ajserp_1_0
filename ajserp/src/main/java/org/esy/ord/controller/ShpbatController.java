...
import org.esy.ord.service.IShpbatService;
 ...
 */
@Controller
@RequestMapping("/api/ord/shpbat")
public class ShpbatController {

    public static final String AUTHORITY = "ord_shpbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IShpbatService shpbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Shpbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", shpbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Shpbat o = shpbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Shpbat, pageable
	 * @return HttpResult
	 * @date Thu Jul 30 08:48:09 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Shpbat shpbat, Pageable pageable) {
		if (shpbat == null) {
			shpbat = new Shpbat();
		}
		return new HttpResult(shpbatService.query(shpbat, pageable));
	}
