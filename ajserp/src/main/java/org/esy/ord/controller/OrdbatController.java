...
public class OrdbatController {
	...
	@Autowired
	private IOrdbatService ordbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Ordbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", ordbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Ordbat o = ordbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Ordbat, pageable
	 * @return HttpResult
	 * @date Tue Jul 28 11:22:38 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Ordbat ordbat, Pageable pageable) {
		if (ordbat == null) {
			ordbat = new Ordbat();
		}
		return new HttpResult(ordbatService.query(ordbat, pageable));
	}
