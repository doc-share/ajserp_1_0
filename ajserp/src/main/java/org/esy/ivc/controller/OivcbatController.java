...
import org.esy.ivc.service.IOivcbatService;
 ...
 */
@Controller
@RequestMapping("/api/ivc/oivcbat")
public class OivcbatController {

    public static final String AUTHORITY = "ivc_oivcbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IOivcbatService oivcbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Oivcbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", oivcbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Oivcbat o = oivcbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Oivcbat, pageable
	 * @return HttpResult
	 * @date Thu Jul 30 14:45:56 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Oivcbat oivcbat, Pageable pageable) {
		if (oivcbat == null) {
			oivcbat = new Oivcbat();
		}
		return new HttpResult(oivcbatService.query(oivcbat, pageable));
	}
