 ...
 */
@Controller
@RequestMapping("/api/ivc/oivcbah")
public class OivcbahController {

    public static final String AUTHORITY = "ivc_mapdo";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IOivcbahService oivcbahService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Oivcbahv o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", oivcbahService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Oivcbahv o = oivcbahService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Oivcbah, pageable
	 * @return HttpResult
	 * @date Thu Jul 30 14:46:11 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Oivcbahv oivcbahv, Pageable pageable) {
		if (oivcbahv == null) {
			oivcbahv = new Oivcbahv();
		}
		return new HttpResult(oivcbahService.query(oivcbahv, pageable));
	}
