package org.esy.nms.controller;
 ...
 */
@Controller
@RequestMapping("/api/nms/othebook")
public class OthebookController {

    public static final String AUTHORITY = "nms_mapz5";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IOthebookService othebookService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Othebook o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", othebookService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Othebook o = othebookService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Othebook, pageable
	 * @return HttpResult
	 * @date Thu Jul 30 11:26:41 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Othebook othebook, Pageable pageable) {
		if (othebook == null) {
			othebook = new Othebook();
		}
		return new HttpResult(othebookService.query(othebook, pageable));
	}
