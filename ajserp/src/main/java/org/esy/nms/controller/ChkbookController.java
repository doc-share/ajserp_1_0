 ...
 */
@Controller
@RequestMapping("/api/nms/chkbook")
public class ChkbookController {

    public static final String AUTHORITY = "nms_mapz3";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IChkbookService chkbookService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Chkbook o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", chkbookService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Chkbook o = chkbookService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Chkbook, pageable
	 * @return HttpResult
	 * @date Thu Jul 30 13:40:35 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Chkbook chkbook, Pageable pageable) {
		if (chkbook == null) {
			chkbook = new Chkbook();
		}
		return new HttpResult(chkbookService.query(chkbook, pageable));
	}
