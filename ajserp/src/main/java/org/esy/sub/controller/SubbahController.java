 ...
 */
@Controller
@RequestMapping("/api/sub/subbah")
public class SubbahController {

    public static final String AUTHORITY = "sub_mape4";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private ISubbahService subbahService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Subbahv o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", subbahService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Subbahv o = subbahService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Subbah, pageable
	 * @return HttpResult
	 * @date Fri Jul 31 09:40:02 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Subbahv subbahv, Pageable pageable) {
		if (subbahv == null) {
			subbahv = new Subbahv();
		}
		return new HttpResult(subbahService.query(subbahv, pageable));
	}
