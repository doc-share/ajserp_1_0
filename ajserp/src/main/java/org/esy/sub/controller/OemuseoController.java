...
import org.esy.sub.service.IOemuseoService;
 ...
 */
@Controller
@RequestMapping("/api/sub/oemuseo")
public class OemuseoController {

    public static final String AUTHORITY = "sub_oemuseo";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IOemuseoService oemuseoService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Oemuseo o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", oemuseoService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Oemuseo o = oemuseoService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Oemuseo, pageable
	 * @return HttpResult
	 * @date Fri Jul 31 16:30:37 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Oemuseo oemuseo, Pageable pageable) {
		if (oemuseo == null) {
			oemuseo = new Oemuseo();
		}
		return new HttpResult(oemuseoService.query(oemuseo, pageable));
	}
