 ...
 */
@Controller
@RequestMapping("/api/sub/subbat")
public class SubbatController {

    public static final String AUTHORITY = "sub_subbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private ISubbatService subbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Subbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", subbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Subbat o = subbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Subbat, pageable
	 * @return HttpResult
	 * @date Fri Jul 31 09:39:52 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Subbat subbat, Pageable pageable) {
		if (subbat == null) {
			subbat = new Subbat();
		}
		return new HttpResult(subbatService.query(subbat, pageable));
	}
