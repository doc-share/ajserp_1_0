 ...
 */
@Controller
@RequestMapping("/api/bas/purven")
public class PurvenController {

    public static final String AUTHORITY = "bas_map12";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IPurvenService purvenService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Purven o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", purvenService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Purven o = purvenService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Purven, pageable
	 * @return HttpResult
	 * @date Fri Jul 24 09:29:45 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Purven purven, Pageable pageable) {
		if (purven == null) {
			purven = new Purven();
		}
		return new HttpResult(purvenService.query(purven, pageable));
	}
