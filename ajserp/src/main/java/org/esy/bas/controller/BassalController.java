 ...
 */
@Controller
@RequestMapping("/api/bas/bassal")
public class BassalController {

    public static final String AUTHORITY = "bas_map16";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IBassalService bassalService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Bassal o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", bassalService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Bassal o = bassalService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Bassal, pageable
	 * @return HttpResult
	 * @date Thu Jul 23 13:06:35 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Bassal bassal, Pageable pageable) {
		if (bassal == null) {
			bassal = new Bassal();
		}
		return new HttpResult(bassalService.query(bassal, pageable));
	}
