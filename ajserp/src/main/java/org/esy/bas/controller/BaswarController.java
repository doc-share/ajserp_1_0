 ...
 */
@Controller
@RequestMapping("/api/bas/baswar")
public class BaswarController {

    public static final String AUTHORITY = "bas_map183";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IBaswarService baswarService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Baswar o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", baswarService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Baswar o = baswarService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Baswar, pageable
	 * @return HttpResult
	 * @date Thu Jul 23 14:10:56 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Baswar baswar, Pageable pageable) {
		if (baswar == null) {
			baswar = new Baswar();
		}
		return new HttpResult(baswarService.query(baswar, pageable));
	}
