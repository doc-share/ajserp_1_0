 ...
 */
@Controller
@RequestMapping("/api/bas/accdept")
public class AccdeptController {

    public static final String AUTHORITY = "bas_mapacc14";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IAccdeptService accdeptService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Accdept o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", accdeptService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Accdept o = accdeptService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Accdept, pageable
	 * @return HttpResult
	 * @date Thu Jul 23 13:03:27 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Accdept accdept, Pageable pageable) {
		if (accdept == null) {
			accdept = new Accdept();
		}
		return new HttpResult(accdeptService.query(accdept, pageable));
	}
