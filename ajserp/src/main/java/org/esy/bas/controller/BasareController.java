...
import org.esy.bas.service.IBasareService;
 ...
 */
@Controller
@RequestMapping("/api/bas/basare")
public class BasareController {

    public static final String AUTHORITY = "bas_map184";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IBasareService basareService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Basare o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", basareService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Basare o = basareService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Basare, pageable
	 * @return HttpResult
	 * @date Thu Jul 23 14:05:10 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Basare basare, Pageable pageable) {
		if (basare == null) {
			basare = new Basare();
		}
		return new HttpResult(basareService.query(basare, pageable));
	}
