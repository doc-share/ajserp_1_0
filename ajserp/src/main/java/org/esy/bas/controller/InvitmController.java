 ...
 */
@Controller
@RequestMapping("/api/bas/invitm")
public class InvitmController {

    public static final String AUTHORITY = "bas_map14";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IInvitmService invitmService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Invitm o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", invitmService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Invitm o = invitmService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Invitm, pageable
	 * @return HttpResult
	 * @date Fri Jul 24 11:01:41 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Invitm invitm, Pageable pageable) {
		if (invitm == null) {
			invitm = new Invitm();
		}
		return new HttpResult(invitmService.query(invitm, pageable));
	}
