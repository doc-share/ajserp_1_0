 ...
 */
@Controller
@RequestMapping("/api/bas/baspro")
public class BasproController {

    public static final String AUTHORITY = "bas_map18e";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IBasproService basproService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Baspro o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", basproService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Baspro o = basproService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Baspro, pageable
	 * @return HttpResult
	 * @date Fri Jul 24 10:18:40 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Baspro baspro, Pageable pageable) {
		if (baspro == null) {
			baspro = new Baspro();
		}
		return new HttpResult(basproService.query(baspro, pageable));
	}
