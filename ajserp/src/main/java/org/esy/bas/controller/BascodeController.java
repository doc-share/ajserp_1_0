 ...
 */
@Controller
@RequestMapping("/api/bas/bascode")
public class BascodeController {

    public static final String AUTHORITY = "bas_map18x";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IBascodeService bascodeService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Bascode o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", bascodeService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Bascode o = bascodeService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Bascode, pageable
	 * @return HttpResult
	 * @date Thu Jul 23 13:25:39 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Bascode bascode, Pageable pageable) {
		if (bascode == null) {
			bascode = new Bascode();
		}
		return new HttpResult(bascodeService.query(bascode, pageable));
	}
