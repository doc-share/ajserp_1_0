 ...
 */
@Controller
@RequestMapping("/api/bas/basun")
public class BasunController {

    public static final String AUTHORITY = "bas_map153";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IBasunService basunService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Basun o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", basunService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Basun o = basunService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Basun, pageable
	 * @return HttpResult
	 * @date Thu Jul 23 12:03:47 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Basun basun, Pageable pageable) {
		if (basun == null) {
			basun = new Basun();
		}
		return new HttpResult(basunService.query(basun, pageable));
	}
