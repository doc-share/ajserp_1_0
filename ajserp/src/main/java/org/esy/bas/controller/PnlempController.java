...
import org.esy.bas.service.IPnlempService;
 ...
 */
@Controller
@RequestMapping("/api/bas/pnlemp")
public class PnlempController {

    public static final String AUTHORITY = "bas_mapsalt";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IPnlempService pnlempService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Pnlemp o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", pnlempService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Pnlemp o = pnlempService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Pnlemp, pageable
	 * @return HttpResult
	 * @date Thu Jul 23 15:09:54 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Pnlemp pnlemp, Pageable pageable) {
		if (pnlemp == null) {
			pnlemp = new Pnlemp();
		}
		return new HttpResult(pnlempService.query(pnlemp, pageable));
	}
