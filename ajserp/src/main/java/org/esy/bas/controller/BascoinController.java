 ...
 */
@Controller
@RequestMapping("/api/bas/bascoin")
public class BascoinController {

    public static final String AUTHORITY = "bas_map18f";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IBascoinService bascoinService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Bascoin o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", bascoinService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Bascoin o = bascoinService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Bascoin, pageable
	 * @return HttpResult
	 * @date Thu Jul 23 14:48:07 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Bascoin bascoin, Pageable pageable) {
		if (bascoin == null) {
			bascoin = new Bascoin();
		}
		return new HttpResult(bascoinService.query(bascoin, pageable));
	}
