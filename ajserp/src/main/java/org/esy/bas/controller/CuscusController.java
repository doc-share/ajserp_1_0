...
public class CuscusController {
	...
	@Autowired
	private ICuscusService cuscusService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Cuscus o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", cuscusService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Cuscus o = cuscusService.getByUid(uid);
		if (o == null) {
	...
	public HttpResult query(@Valid @RequestBody(required = false) Cuscus cuscus, Pageable pageable) {
		...
		return new HttpResult(cuscusService.query(cuscus, pageable));
	}
