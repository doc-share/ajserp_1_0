...
public class BasfacController {
	...
	@Autowired
	private IBasfacService basfacService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Basfac o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", basfacService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Basfac o = basfacService.getByUid(uid);
		if (o == null) {
	...
	public HttpResult query(@Valid @RequestBody(required = false) Basfac basfac, Pageable pageable) {
		...
		return new HttpResult(basfacService.query(basfac, pageable));
	}
