...
public class AcccdController {
	...
	@Autowired
	private IAcccdService acccdService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Acccd o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", acccdService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Acccd o = acccdService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Acccd, pageable
	 * @return HttpResult
	 * @date Sat Jun 20 08:54:05 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Acccd acccd, Pageable pageable) {
		if (acccd == null) {
			acccd = new Acccd();
		}
		return new HttpResult(acccdService.query(acccd, pageable));
	}
