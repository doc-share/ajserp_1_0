 ...
 */
@Controller
@RequestMapping("/api/inv/antbat")
public class AntbatController {

    public static final String AUTHORITY = "inv_antbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IAntbatService antbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Antbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", antbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Antbat o = antbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Antbat, pageable
	 * @return HttpResult
	 * @date Mon Aug 03 08:52:45 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Antbat antbat, Pageable pageable) {
		if (antbat == null) {
			antbat = new Antbat();
		}
		return new HttpResult(antbatService.query(antbat, pageable));
	}
