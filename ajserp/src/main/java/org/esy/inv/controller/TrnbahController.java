 ...
 */
@Controller
@RequestMapping("/api/inv/trnbah")
public class TrnbahController {

    public static final String AUTHORITY = "inv_map64";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private ITrnbahService trnbahService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Trnbahv o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", trnbahService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Trnbahv o = trnbahService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Trnbah, pageable
	 * @return HttpResult
	 * @date Mon Aug 03 09:09:25 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Trnbahv trnbahv, Pageable pageable) {
		if (trnbahv == null) {
			trnbahv = new Trnbahv();
		}
		return new HttpResult(trnbahService.query(trnbahv, pageable));
	}
