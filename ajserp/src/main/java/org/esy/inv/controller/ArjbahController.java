 ...
 */
@Controller
@RequestMapping("/api/inv/arjbah")
public class ArjbahController {

    public static final String AUTHORITY = "inv_map62";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IArjbahService arjbahService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Arjbahv o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", arjbahService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Arjbahv o = arjbahService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Arjbah, pageable
	 * @return HttpResult
	 * @date Fri Jul 31 17:24:35 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Arjbahv arjbahv, Pageable pageable) {
		if (arjbahv == null) {
			arjbahv = new Arjbahv();
		}
		return new HttpResult(arjbahService.query(arjbahv, pageable));
	}
