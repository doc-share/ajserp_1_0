 ...
 */
@Controller
@RequestMapping("/api/inv/invtra")
public class InvtraController {

    public static final String AUTHORITY = "inv_invtra";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IInvtraService invtraService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Invtra o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", invtraService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Invtra o = invtraService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Invtra, pageable
	 * @return HttpResult
	 * @date Mon Aug 03 10:12:58 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Invtra invtra, Pageable pageable) {
		if (invtra == null) {
			invtra = new Invtra();
		}
		return new HttpResult(invtraService.query(invtra, pageable));
	}
