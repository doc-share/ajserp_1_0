 ...
 */
@Controller
@RequestMapping("/api/inv/trnbat")
public class TrnbatController {

    public static final String AUTHORITY = "inv_trnbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private ITrnbatService trnbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Trnbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", trnbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Trnbat o = trnbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Trnbat, pageable
	 * @return HttpResult
	 * @date Mon Aug 03 09:09:21 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Trnbat trnbat, Pageable pageable) {
		if (trnbat == null) {
			trnbat = new Trnbat();
		}
		return new HttpResult(trnbatService.query(trnbat, pageable));
	}
