 ...
 */
@Controller
@RequestMapping("/api/inv/arjbat")
public class ArjbatController {

    public static final String AUTHORITY = "inv_arjbat";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IArjbatService arjbatService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Arjbat o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", arjbatService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Arjbat o = arjbatService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Arjbat, pageable
	 * @return HttpResult
	 * @date Fri Jul 31 17:24:38 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Arjbat arjbat, Pageable pageable) {
		if (arjbat == null) {
			arjbat = new Arjbat();
		}
		return new HttpResult(arjbatService.query(arjbat, pageable));
	}
