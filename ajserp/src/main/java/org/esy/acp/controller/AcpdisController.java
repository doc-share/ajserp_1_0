 ...
 */
@Controller
@RequestMapping("/api/acp/acpdis")
public class AcpdisController {

    public static final String AUTHORITY = "acp_acpdis";

	@Autowired
	private ILoginService loginService;

	@Autowired
	private IAcpdisService acpdisService;
	...
	@ResponseBody
	public ResponseEntity<Response> save(@RequestBody Acpdis o, HttpServletRequest request) {
		...
		try {
			//o.setUid(null);
			resp = new Response(0, "Save success.", acpdisService.save(o));
	...
	public ResponseEntity<Response> get(@PathVariable String uid, HttpServletRequest request) {
		...
		Acpdis o = acpdisService.getByUid(uid);
		if (o == null) {
	 ...
	 * 
	 * @author <a href="mailto:ardui@163.com">ardui</a>
	 * @param Acpdis, pageable
	 * @return HttpResult
	 * @date Thu Jul 30 09:45:23 CST 2020	
	 */
	@RequestMapping(value = "query", method = RequestMethod.POST)
	public HttpResult query(@Valid @RequestBody(required = false) Acpdis acpdis, Pageable pageable) {
		if (acpdis == null) {
			acpdis = new Acpdis();
		}
		return new HttpResult(acpdisService.query(acpdis, pageable));
	}
